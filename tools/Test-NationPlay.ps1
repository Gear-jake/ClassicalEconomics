...
Assert ($tsw -match 'NationEngine\.IsMarketCity') 'Trade capacity must apply the market bonus'
Assert ($tsw -match 'NationEngine\.PactFlowMult') 'Trade flows must apply the trade-pact multiplier'
Assert ($tsw -match 'NationEngine\.TariffImportMult') 'Trade flows must apply the tariff import multiplier'
Assert ($tsw -match 'NationEngine\.TariffPriceMult') 'Local price must apply the tariff price multiplier'
...
if (-not (Test-Path -LiteralPath $diplomacyPath -PathType Leaf)) {
    ...
    Assert ($dip -match 'BilateralFlowMult') 'diplomacy must expose bilateral pact flow multiplier'
