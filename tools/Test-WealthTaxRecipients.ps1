...
$pattern = 'poor\.Clear\(\).*rich\.Clear\(\).*foreach \(var actor in AllCivPool\).*if \(w < poorLine\) poor\.Add\(actor\);.*else if \(w > taxLine\) rich\.Add\(actor\);.*if \(poor\.Count == 0\) return;.*long totalTax = 0;.*foreach \(var actor in rich\).*actor\.addMoney\(-charged\)'
