...
$stageOrder = 'enum AnnualStage\s*\{[^}]*WealthTax,\s*CycleModulator,\s*Unrest,\s*Policy,\s*KingdomMonitor,\s*SocialCrisis,\s*Population,\s*Spending,\s*EraEvaluate,\s*EraTick,\s*Disaster,\s*Banking,\s*Bank,\s*Nation,\s*Events,\s*Snapshot,\s*Done'
Test-Anchor $pipeline $stageOrder 'AnnualStage enum must declare the closeout order (WealthTax..Banking, Bank, Nation, Events, Snapshot, Done)'
