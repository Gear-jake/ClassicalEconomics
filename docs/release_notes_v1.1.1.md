...
> 状态说明：兴建建筑改为放置**游戏原版建筑**（房子/兵营/水井等， 式点击地图放置）；45/45 门禁 + performance_audit + 构建全绿。

## 原版建筑放置（点击地图放置模式 同款交互）

- **用法**：内阁 →「法令·建设」页 →「兴建原版建筑」区选一种建筑（基础房屋/标准房屋/豪华住宅/兵营/瞭望塔/水井/矿井/纪念碑/神庙/篝火）→ 进入放置模式 → **鼠标点击本国领土**即建造（连续放置，无需反复进页）→ **右键取消**。
- **建筑目录**：读取原版 `BuildingAsset`（含 cost 字段）与库；显示 10 种常用建筑；房屋按种族拼接 ID（`house_human_3`，与  的 GetRaceBuild 同规则），通用建筑（水井/矿井/纪念碑/篝火）直接使用原版 ID；原版无对应资产时 fail-closed 提示。
